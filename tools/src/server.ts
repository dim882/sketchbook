...
async function loadCSSModulesMapping() {
  try {
...
// Initialize CSS modules mapping
let styles: any = {};

loadCSSModulesMapping()
  .then((mapping) => {
    styles = mapping;
  })
  .catch((err) => {
...
async function renderMainPage(sketchName?: string) {
  ...
  const sketchListHtml = render(h(SketchListWithStyles, { dirs: sortedDirs }));
